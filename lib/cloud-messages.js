...
module.exports.sendCloudMessage = function sendCloudMessage(options, next) {
  ...
  var message = {
    ...
    ttl           : options.ttl || 900,
    data          : options.data || '{}',
    transId       :options.transId || null,
    transactionId :options.transId || null
  };

  if (options.recipientClassName) { message.recipient.className == options.recipientClassName; }
  if (options.recipientPackageName) { message.recipient.packageName == options.recipientPackageName; }
  if (options.transId) { message.transId = options.transId }
  if (options.multiTender) { message.multiTender = options.multiTender; }
